classification_purity_init <- function(y, offset, parms, wt) {
  #taken from rpart
  ...
  if (missing(parms) || is.null(parms))
    ...
    if (is.null(parms$split)) {
      ...
      stop("Invalid 'split' parameter: class purity does not use gini or information.")
    }
...
classification_purity_eval <- function(y, wt, parms) {
  y <- as.factor(y)
  y.labels <- levels(y)
  y.tab <- table(y)
